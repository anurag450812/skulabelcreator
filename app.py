...
WHATSAPP_UPLOAD_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'whatsapp_uploads')
os.makedirs(WHATSAPP_UPLOAD_DIR, exist_ok=True)
...
# --- WhatsApp Salary Extractor ---
WHATSAPP_DATE_RE = re.compile(
...
def parse_whatsapp_file(filepath):
    ...
    for raw in raw_lines:
        line = raw.rstrip('\n')
        match = WHATSAPP_DATE_RE.match(line)
        if match:
            if current_msg:
                messages.append(current_msg)
            date_str, time_str = match.group(1), match.group(2)
            sender_and_text = line[match.end():]
            colon_idx = sender_and_text.find(': ')
            if colon_idx != -1:
                sender = sender_and_text[:colon_idx]
                text = sender_and_text[colon_idx + 2:]
            else:
                sender = ''
                text = sender_and_text
            ...
            current_msg = {
                ...
                'sender': sender,
                'lines': [text],
                'line_numbers': [i + 1 for i, _ in enumerate([])],
            }
            current_msg['line_numbers'] = [raw_lines.index(raw) + 1]
        else:
            if current_msg:
                current_msg['lines'].append(line)
                current_msg['line_numbers'].append(raw_lines.index(raw) + 1)
            else:
                messages.append({
                    'date_str': '',
                    'time_str': '',
                    ...
                    'sender': '',
                    'lines': [line],
                    'line_numbers': [raw_lines.index(raw) + 1],
                })
    if current_msg:
        messages.append(current_msg)
    return messages, raw_lines

def get_whatsapp_stored_path():
    candidates = glob.glob(os.path.join(WHATSAPP_UPLOAD_DIR, '*.txt'))
...
def whatsapp_upload():
    ...
    if not chatfile.filename:
        return jsonify({'error': 'No file selected'}), 400
    if not chatfile.filename.lower().endswith('.txt'):
        return jsonify({'error': 'Only .txt files are supported'}), 400

    old = get_whatsapp_stored_path()
    if old:
        os.remove(old)

    save_path = os.path.join(WHATSAPP_UPLOAD_DIR, chatfile.filename)
    chatfile.save(save_path)

    messages, raw_lines = parse_whatsapp_file(save_path)
    return jsonify({
...
def whatsapp_status():
    path = get_whatsapp_stored_path()
    if not path:
        return jsonify({'loaded': False})
    messages, raw_lines = parse_whatsapp_file(path)
    return jsonify({
        'loaded': True,
...
def whatsapp_search():
    path = get_whatsapp_stored_path()
    if not path:
        return jsonify({'error': 'No chat file uploaded yet'}), 400
    ...
    for msg in messages:
        ...
        if term.lower() in full_text.lower():
            header = f"{msg['date_str']}, {msg['time_str']} - {msg['sender']}" if msg['date_str'] else ''
            results.append({
                'header': header,
                'lines': msg['lines'],
                'line_numbers': msg['line_numbers'],
            })

    return jsonify({'results': results, 'count': len(results), 'term': term})

@app.route('/whatsapp/trim', methods=['POST'])
def whatsapp_trim():
    path = get_whatsapp_stored_path()
    if not path:
        return jsonify({'error': 'No chat file uploaded yet'}), 400
    ...
    data = request.get_json()
    months = float(data.get('months', 1.5))
    ...
    kept_indices = set()
    for msg in messages:
        if msg['datetime'] is None or msg['datetime'] >= cutoff:
            for ln in msg['line_numbers']:
                kept_indices.add(ln)

    trimmed_lines = []
    for i, raw in enumerate(raw_lines):
        if (i + 1) in kept_indices:
            trimmed_lines.append(raw)

    removed = len(raw_lines) - len(trimmed_lines)
    filename = os.path.basename(path)
    name_no_ext = os.path.splitext(filename)[0]

    tmp_path = os.path.join(tempfile.gettempdir(), f"{name_no_ext}_trimmed_{uuid.uuid4().hex[:8]}.txt")
    with open(tmp_path, 'w', encoding='utf-8') as f:
        f.writelines(trimmed_lines)

    return send_file(tmp_path, as_attachment=True,
                     download_name=f'{name_no_ext}_trimmed.txt',
                     mimetype='text/plain')

@app.route('/whatsapp/delete', methods=['DELETE'])
def whatsapp_delete():
    path = get_whatsapp_stored_path()
    if path:
        os.remove(path)
        return jsonify({'deleted': True})
    return jsonify({'deleted': False, 'error': 'No file to delete'}), 404
